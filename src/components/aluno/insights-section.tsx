...
const URGENCIA_CONFIG: Record<InsightUrgencia, { label: string; cardClass: string; badgeVariant: BadgeVariant }> = {
  alta:     { label: "Urgente",      cardClass: "border-l-4 border-l-red-500 bg-red-50/50 dark:bg-red-950/20",      badgeVariant: "destructive" },
  media:    { label: "Atenção",      cardClass: "border-l-4 border-l-amber-500 bg-amber-50/50 dark:bg-amber-950/20", badgeVariant: "warning" },
  baixa:    { label: "Informativo",  cardClass: "border-l-4 border-l-blue-400 bg-blue-50/50 dark:bg-blue-950/20",   badgeVariant: "secondary" },
  positivo: { label: "Oportunidade", cardClass: "border-l-4 border-l-emerald-500 bg-emerald-50/50 dark:bg-emerald-950/20", badgeVariant: "success" },
};
...
function tendenciaIcon(tendencia: string): string {
...
function InsightCard({ insight }: { insight: Insight }) {
  ...
  return (
    <div className={`rounded-lg p-4 space-y-3 ${cfg.cardClass}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <Badge variant={cfg.badgeVariant} className="text-xs whitespace-nowrap">{cfg.label}</Badge>
      ...
      {insight.mensagem_sugerida && (
        <div className="rounded-md bg-background/70 border border-border/50 px-3 py-2">
          <p className="text-xs font-medium text-muted-foreground mb-0.5 flex items-center gap-1">
            <MessageCircle className="h-3 w-3" /> Mensagem sugerida
          </p>
...
export async function InsightsSection({ alunoId, aluno }: Props) {
  ...
  let scores: ScoresIA | null = null;
  let protocolo: ProtocoloIA | null = null;
  let riscos: RiscosIA | null = null;
  ...
  try {
    ...
    anamnese = (anamneseRes.data ?? null) as Anamnese | null;
    checkins = (checkinsRes.data ?? []) as Checkin[];
    const rel = relatorioRes.data;
    if (rel) {
      scores = (rel.scores ?? null) as ScoresIA | null;
      protocolo = (rel.protocolo_mobilidade ?? null) as ProtocoloIA | null;
      riscos = (rel.riscos ?? null) as RiscosIA | null;
    }
  } catch (err) {
    console.error("InsightsSection: erro ao buscar dados", err);
    return (
      <Card>
  ...
  const insights = gerarInsights({
    ...
    anamnese,
    checkins,
    relatorioScores: scores,
    relatorioProtocoloMobilidade: protocolo,
    relatorioRiscos: riscos,
  });

  const score = calcularScoreEvolucao(checkins, scores);
        ...
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center justify-between">
            ...
            <span className={`text-2xl font-bold ${scoreColor(score.total)}`}>
              {score.total}/100{" "}
              <span className="text-sm font-normal">{tendenciaIcon(score.tendencia)}</span>
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          {score.tendencia === "sem_dados" && (
            <p className="text-xs text-muted-foreground text-center">
              Score baseado no relatório IA. Será atualizado com os check-ins.
            </p>
          )}
          {score.tendencia === "caindo" && (
            <p className="text-xs text-amber-600 text-center font-medium">
              Tendência de queda nos últimos check-ins. Atenção recomendada.
            </p>
          )}
          {score.tendencia === "subindo" && (
            <p className="text-xs text-emerald-600 text-center font-medium">
              Tendência de melhora nos últimos check-ins.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {insights.length === 0 && (
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground text-sm">
            Nenhum insight disponível ainda. Complete a anamnese e registre alguns check-ins.
          </CardContent>
        </Card>
      )}
